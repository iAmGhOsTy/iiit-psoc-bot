const { Spotify, AppleMusic,SoundCloud } = require('shoukaku-sources');
const TwentyFourSeven = require('../database/schemas/247');
...
class Song {
  constructor(track, user) {
    ...
    if (track.info.sourceName === 'youtube') {
      ...
      new SoundCloud().getTrack(track.info.uri).then((res) => {
        this.info.thumbnail = res.artwork_url;
      });
    }
  }
}
...
class Dispatcher {
    ...
    this.player
      .on('start', () => {
        try {
          ...
          console.error('Error in trackStart event handler:', error);
        }
      })
      .on('end', () => {
        if (!this.queue.length)
          this.client.shoukaku.emit('queueEnd', this.player, this.current, this);
        this.client.shoukaku.emit('trackEnd', this.player, this.current);
      })
  ...
  destroy() {
    this.queue.length = 0;
    this.history = [];
    if (this.player && this.player.connection) {
      // Disconnect only if there is a connection
  ...
  async disconnect() {
    ...
    const _247 = await TwentyFourSeven.findOne({ guildId: this.guildId });
